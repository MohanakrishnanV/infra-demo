resource "aws_instance" "csf_dev_vm" {
  ami                     = var.dev_vm_ami
  instance_type           = var.dev_vm_instance_type
  key_name                = var.dev_vm_keyname
  vpc_security_group_ids  = var.dev_security_group_ids
  subnet_id               = var.dev_vm_subnet_id
  disable_api_termination = true
  root_block_device {
    volume_size           = var.dev_vm_os_disk_size
    volume_type           = "gp3"
...
resource "terraform_data" "setup_authorized_keys" {
  provisioner "file" {
    ...
    connection {
      type        = "ssh"
      host        = aws_instance.csf_dev_vm.private_ip
      user        = var.csf_dev_vm_username
      private_key = file(var.csf_dev_vm_private_key_path)
    }
  }
  ...
  depends_on = [aws_instance.csf_dev_vm]
}
...
data "external" "jenkins_pubkey" {
  program = ["powershell", "-Command", <<-EOC
    $pubkey = ssh -o StrictHostKeyChecking=no -i '${var.csf_dev_vm_private_key_path}' ${var.csf_dev_vm_username}@${aws_instance.csf_dev_vm.private_ip} 'sudo cat /home/jenkins/.ssh/id_rsa.pub'
...
resource "terraform_data" "nginx_conf_setup" {
  provisioner "file" {
    source      = "${path.module}/dev-csf.conf"
    destination = "/tmp/dev-csf.conf"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo mv /tmp/dev-csf.conf /etc/nginx/sites-available/dev-csf.conf",
      "sudo ln -sf /etc/nginx/sites-available/dev-csf.conf /etc/nginx/sites-enabled/dev-csf.conf",
      "sudo nginx -t",
  ...
  triggers_replace = {
    conf_hash = filesha256("${path.module}/dev-csf.conf")
  }

  depends_on = [terraform_data.install_nginx]
}

resource "aws_lb_target_group" "csf_dev_vm_lb_tg" {
  name        = var.csf_dev_vm_lb_tg_name
  port        = 80
  protocol    = "HTTP"
...
resource "aws_lb_target_group_attachment" "csf_dev_vm_lb_tg_attach" {
  target_group_arn = aws_lb_target_group.csf_dev_vm_lb_tg.arn
  target_id        = aws_instance.csf_dev_vm.id
  port             = 80
  depends_on       = [aws_lb_target_group.csf_dev_vm_lb_tg, aws_instance.csf_dev_vm]
}

resource "aws_lb_listener_rule" "csf_dev_vm_lb_listn_rule" {
  listener_arn = var.dev_lb_listn_arn
  priority     = 200
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.csf_dev_vm_lb_tg.arn
  ...
  condition {
    host_header {
      values = [var.csf_dev_domain]
    }
  }
  tags = {
    Name = var.csf_dev_lb_listn_rule_name
  }
  depends_on = [aws_lb_target_group.csf_dev_vm_lb_tg]
}
